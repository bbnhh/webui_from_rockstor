...
from fs.btrfs import qgroup_exists,zfs_share_usage
...
class Share(models.Model):
    ...
    poolname = models.CharField(max_length=4096,blank=True, null=True)
    """auto created 0/x qgroup"""
    qgroup = models.CharField(max_length=100,blank=True, null=True)
    """quota group y/x explicitly created for this Share"""
    pqgroup = models.CharField(max_length=32,
                               default=settings.MODEL_DEFS['pqgroup'],blank=True, null=True)
    ...
    @property
    def share_used(self):
        used = zfs_share_usage(self.poolname,self.name)
        return used

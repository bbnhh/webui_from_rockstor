...
class ShareListView(ShareMixin, rfc.GenericView):
    ...
    def post(self, request):
        ...
        with self._handle_exception(request):
            ...
            add_share(pool, sname, size)
            #qid = qgroup_id(pool, sname)
            s = Share(pool=pool, poolname=pool.name ,qgroup='', pqgroup='', name=sname,
            ...
            logger.debug("LLLLCREATEKKKKK")
            logger.debug(ShareSerializer(s))
            pool.save()
            s.save()
            '''

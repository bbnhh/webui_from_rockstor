...
class NFSExportMixin(object):
            ...
            for f in fields[1:]:
                ...
                exports_d[fields[0]].append(
                    {'client_str': cf[0], 'option_list': cf[1][:-1],
                     'mnt_pt': ('%s%s' % (settings.MNT_PT, share.poolname))})
        logger.debug(exports_d)
        return exports_d
    ...
    def create_nfs_export_input(cls, exports):
        ...
        for e in exports:
            e_list = []
            export_pt = ('%s%s/%s' % (settings.NFS_EXPORT_ROOT, e.share.poolname, e.share.name))
...
class NFSExportGroupDetailView(NFSExportMixin, rfc.GenericView):
    ...
    def put(self, request, export_id):
        ...
        with self._handle_exception(request):
            ...
            for s in shares:
                mnt_pt = ('%s%s/%s' % (settings.MNT_PT, s.name, s.poolname))
                #export_pt = ('%s%s/%s' % (settings.NFS_EXPORT_ROOT, s.name, s.poolname))
                export_pt = ('%s%s/%s' % (settings.NFS_EXPORT_ROOT, s.poolname, s.name))
                #if not s.is_mounted:
                #    mount_share(s, mnt_pt)
                real_name = '/%s/%s' %(s.poolname, s.name)
                logger.debug('NFSSSSSSSSSSSSSSSSS')
                export = NFSExport(export_group=eg, share=s, mount=export_pt)
                export.full_clean()
                export.save()
                cur_exports.append(export)
...
class AdvancedNFSExportView(NFSExportMixin, rfc.GenericView):
    ...
    def post(self, request):
        with self._handle_exception(request):
            ...
            cur_exports = list(NFSExport.objects.all())
            logger.debug('NfsVieWWWWWWWW: %s' % cur_exports)
            exports = self.create_nfs_export_input(cur_exports)
            exports.update(exports_d)
            self.refresh_wrapper(exports, request, logger)

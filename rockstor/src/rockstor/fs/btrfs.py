...
from system.osi import run_command, create_tmp_dir, is_share_mounted, \
    ...
    get_device_path, tgmk_rvs, shell_call_rc
...
def del_pool(pool):
    ...
    if rc == 0:
        ...
        logger.error('Unknown state in del_pool() - non-zero code (%d) '
...
def get_pool_info(disk, root_pool=False):
    ...
    logger.debug('POOLLLLLLL' % pool_info)
    return pool_info
...
def add_share(pool, share_name, size):
    """
    ...
    '''
    subvol_mnt_pt = pool.name + '/' + share_name
    size_cmd = "quota=%s" %(size*1024) 
    sub_vol_cmd = [ZFS, 'create', '-o', size_cmd ,subvol_mnt_pt]
...
def share_id(pool, share_name):
    ...
    out, err, rc = run_command([ZFS, 'list'])
    subvol_id = None
    for line in out:
...
def remove_share(pool, share_name, pqgroup, force=False):
    ...
    '''
    del_name = pool.name+'/'+share_name
    run_command([ZFS, 'destroy', del_name], log=True)

def remove_snap(pool, share_name, snap_name, snap_qgroup):
    
    snap = pool.name+'/'+share_name+'@'+snap_name
    o, e, rc = run_command([ZFS, 'destroy', snap])
...
def add_snap_helper(pool_name, share_name, snap_name):
    snapname = pool_name + '/' + share_name + '@' + snap_name
    cmd = [ZFS, 'snap', snapname]
    logger.debug('SnaPPPPP: %s' % (cmd))
...
def add_snap(pool, share_name, snap_name, writable):
    ...
    return add_snap_helper(pool.name, share_name, snap_name)
...
def shares_usage(pool, share_map, snap_map):
    ...
    logger.debug('SharEEEEE' % usage_map)
    return usage_map



def zfs_share_usage(pool_name,share_name):
    ...
    """
    name = pool_name + '/' + share_name
#    cmd = [BTRFS, 'fi', 'usage', '-b', mnt_pt]
    #"zpool list -H -o free po"
    cmd = [ZFS, 'list', '-H', '-o', 'used', name]

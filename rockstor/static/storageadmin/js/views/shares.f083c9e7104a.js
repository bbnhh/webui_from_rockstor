...
//	delete button handler
    ...
    initHandlebarHelpers: function(){
        ...
        //});
        Handlebars.registerHelper('humanize_size', function(type, size, usage) {
            //alert("xxxxxxxxx")
            var html = '';
            if(type == 'size'){
                html += humanize.filesize(size * 1024);
            }else if(type == 'usage'){
                html += humanize.filesize(usage * 1024);
                //alert(size)
                //alert(poolFree)
                //alert(size * 1024 - poolFree)
                //html += humanize.filesize(size *1024 - poolFree);
            //}else if (type == 'usagePercent'){
            }else{
                html += ((size / usage) * 100).toFixed(2);
                //html += (((size * 1024 - poolReclaim - poolFree) / size) * 100).toFixed(2);
            }
